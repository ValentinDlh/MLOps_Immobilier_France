...
def prediction(departement, ville, quartier, vefa, n_pieces, surface_habitable, col):
    ...
    mae_train = median_absolute_error(y_train * df["prix"].std() + df["prix"].mean(),
                                      y_pred_train * df["prix"].std() + df["prix"].mean())
    mae_test = median_absolute_error(y_test * df["prix"].std() + df["prix"].mean(),
                                     y_pred_test * df["prix"].std() + df["prix"].mean())
